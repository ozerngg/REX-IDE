...
Blockly.Blocks['controls_switch'] = {
    ...
    storeConnections_: function (arg) {
        ...
        this.valueConnections_ = [null];
        this.statementConnections_ = [null];
        this.defaultStatementConnection_ = null;
        for (var i = 1; i <= this.caseCount_; i++) {
            if (arg != i) {
                this.valueConnections_.push(this.getInput('SWITCH' + i).connection.targetConnection);
                this.statementConnections_.push(this.getInput('DO' + i).connection.targetConnection);
            }
        }
        if (this.getInput('DEFAULT')) this.defaultStatementConnection_ = this.getInput('DEFAULT').connection.targetConnection;
    },

    restoreConnections_: function () {
        for (var i = 1; i <= this.caseCount_; i++) {
            Blockly.Mutator.reconnect(this.valueConnections_[i], this, 'SWITCH' + i);
            Blockly.Mutator.reconnect(this.statementConnections_[i], this, 'DO' + i);
        }
        if (this.getInput('DEFAULT')) Blockly.Mutator.reconnect(this.defaultStatementConnection_, this, 'DEFAULT');
    },
    addDefault_: function () {
        this.storeConnections_();
        var update = function () {
            this.defaultCount_++;
        };
    ...
    updateShape_: function () {
        ...
        if (this.getInput('DEFAULT')) {
            ...
            this.removeInput('DEFAULTTITLE');
            this.removeInput('DEFAULTBUTTONS');
        }
        var i = 1;
        while (this.getInput('SWITCH' + i)) {
            this.removeInput('SWITCH' + i);
            this.removeInput('SWITCHTITLE' + i);
            this.removeInput('SWITCHBUTTONS' + i);
            this.removeInput('DO' + i);
            i++;
        }
        // Rebuild block.
        for (var i = 1; i <= this.caseCount_; i++) {
            var removeCase = function (arg) {
                return function () {
                    that.removeCase_(arg);
                };
            }(i);
            this.appendValueInput('SWITCH' + i)
                .setCheck(["Number", 'String'])
                .appendField('case');
            this.appendDummyInput('SWITCHTITLE' + i)
                .appendField(Blockly.Msg.CONTROLS_IF_MSG_THEN);
            this.appendDummyInput('SWITCHBUTTONS' + i)
                .appendField(
                    new Blockly.FieldImage(this.REMOVE_IMAGE_DATAURI, 24, 24, "*", removeCase, false))
                .setAlign(Blockly.ALIGN_RIGHT);
            this.appendStatementInput('DO' + i);
        }
        if (this.defaultCount_) {
            this.appendDummyInput('DEFAULTTITLE')
                .appendField('default');
            this.appendDummyInput('DEFAULTBUTTONS')
                .setAlign(Blockly.ALIGN_RIGHT)
                .appendField(
                    new Blockly.FieldImage(this.REMOVE_IMAGE_DATAURI, 24, 24, "*", that.removeDefault_.bind(that), false));
        ...
        var addCase = function () {
            return function () {
                if (that.defaultCount_ == 0) {
                    that.addDefault_();
                } else {
                    if (!that.caseCount_) that.caseCount_ = 0;
                    that.addCase_();
                }
            };
        }();
        this.appendDummyInput('ADDBUTTON')
            .appendField(
                new Blockly.FieldImage(this.ADD_IMAGE_DATAURI, 24, 24, "*", addCase, false));
    },
    ...
    rebuildShape_: function () {
        ...
        var i = 1;
        while (this.getInput('SWITCH' + i)) {
            var inputSwitch = this.getInput('SWITCH' + i);
            var inputDo = this.getInput('DO' + i);
            valueConnections.push(inputSwitch.connection.targetConnection);
            statementConnections.push(inputDo.connection.targetConnection);
            i++;
        }
        this.updateShape_();
        this.reconnectChildBlocks_(valueConnections, statementConnections,
            defaultStatementConnection);
    },
    /**
     * Reconnects child blocks.
     * @param {!Array<?Blockly.RenderedConnection>} valueConnections List of value
     ...
     * @param {!Array<?Blockly.RenderedConnection>} statementConnections List of
     * statement connections for do input.
     * @param {?Blockly.RenderedConnection} defaultStatementConnection Statement
     * connection for else input.
     */
    reconnectChildBlocks_: function (valueConnections, statementConnections,
        defaultStatementConnection) {
        for (var i = 1; i <= this.caseCount_; i++) {
            Blockly.Mutator.reconnect(valueConnections[i], this, 'SWITCH' + i);
            Blockly.Mutator.reconnect(statementConnections[i], this, 'DO' + i);
        }
...
Blockly.defineBlocksWithJsonArray([ // Mutator blocks. Do not extract.
    // Block representing the switch statement in the controls_switch mutator.
    {
        "type": "controls_switch_switch",
        "message0": "switch %1 do",
        "nextStatement": null,
        "enableContextMenu": false,
        "style": "logic_blocks",
    },
    // Block representing the case statement in the controls_switch mutator.
    {
        "type": "controls_switch_switchcase",
        "message0": "case %1 do",
        "previousStatement": null,
        "nextStatement": null,
        "enableContextMenu": false,
    ...
    // Block representing the default statement in the controls_switch mutator.
    {
...
Blockly.Constants.Logic.CONTROLS_SWITCH_MUTATOR_MIXIN = {
    ...
    /**
     * Create XML to represent the number of case and default inputs.
     * @return {Element} XML storage element.
     * @this Blockly.Block
     */
    mutationToDom: function () {
